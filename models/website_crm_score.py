...
class website_crm_score(models.Model):
    ...
    rule_type = fields.Selection(
        [("score", "Scoring"), ("active", "Archive"), ("unlink", "Delete")],
        default="score",
        required=True,
        tracking=True,
    ...
    last_run = fields.Datetime(
        "Last run", help="Date from the last scoring on all leads."
    )
    lead_ids = fields.Many2many(
        "crm.lead", "crm_lead_score_rel", "score_id", "lead_id", string="Leads"
    )
    ...
    def assign_scores_to_leads(self, ids=False, lead_ids=False):
        _logger.info(
            "Start scoring for %s rules and %s leads"
            % (ids and len(ids) or "all", lead_ids and len(lead_ids) or "all")
        )
